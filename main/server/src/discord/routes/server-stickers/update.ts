...
const CLANSOCKET_PERMISSION = "discord:server-stickers.rename";

interface UpdateServerStickerBody {
    userId: string;
    beforeName: string;
    name: string;
...
router.patch(
    ...
    handleAsync(async (req: Request, res: Response) => {
        ...
        const body = req.body as UpdateServerStickerBody;
        ...
        try {
            ...
            recordClanAudit(server.clan_id, {
                actor: body.userId,
                action: ClanAuditActions.DiscordServerStickersRename,
                targetId: stickerId,
                guildId,
                payload: {
                    guildId,
            ...
            logger.error(`[discord] server sticker update failed for ${guildId}/${stickerId}: ${(err as Error).message}`);
            res.status(HTTP_INTERNAL_ERROR).json({ error: "server_sticker_update_failed" });
        }
    }),
);
