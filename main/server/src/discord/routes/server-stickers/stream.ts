...
import { discordServerStickersTopic } from "../../../data-rights/streams/topics/discord-server-stickers-topic.js";
...
router.get("/:guildId/stream", requireSiteAccount, validateGuildId, (req: Request, res: Response) => {
    ...
    const handle = subscribeProjection(
        `discord_server_stickers:${server.clan_id}:${guildId}`,
        discordServerStickersTopic(server.clan_id, guildId),
