...
router.get(
    ...
    handleAsync(async (req: Request, res: Response) => {
        ...
        try {
            const stickers = listServerStickersForGuild(server.clan_id, guildId);
            res.status(HTTP_OK).json({ stickers });
        } catch (err) {
            logger.error(`[discord] server stickers list failed for ${guildId}: ${(err as Error).message}`);
            res.status(HTTP_INTERNAL_ERROR).json({ error: "server_stickers_list_failed" });
        }
    }),
);
