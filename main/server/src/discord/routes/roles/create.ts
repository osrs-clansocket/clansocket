...
const CLANSOCKET_PERMISSION = "discord:roles.create";
    ...
    handleAsync(async (req: Request, res: Response) => {
        ...
        try {
            ...
            recordClanAudit(server.clan_id, {
                actor: body.userId,
                action: ClanAuditActions.DiscordRolesCreate,
                targetId: tempId,
                guildId,
                payload: {
                    guildId,
            ...
            logger.error(`[discord] create role failed for ${guildId}: ${(err as Error).message}`);
            res.status(HTTP_INTERNAL_ERROR).json({ error: "create_role_failed" });
        }
    }),
);
