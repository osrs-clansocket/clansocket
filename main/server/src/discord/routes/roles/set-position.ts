...
const RATE_LIMIT_ROUTE = "/guilds/:id/roles";
const CLANSOCKET_PERMISSION = "discord:roles.set-position";
    ...
    handleAsync(async (req: Request, res: Response) => {
        ...
        try {
            ...
            const changeId = enqueueDraftChange({
                ...
                beforeJson: JSON.stringify({ subject: SUBJECT_POSITION, position: body.beforePosition }),
                afterJson: JSON.stringify({ subject: SUBJECT_POSITION, position: body.afterPosition }),
            });
            const queueId = publishSingleOpDraft({ clanId: server.clan_id, guildId, sessionId });
            recordClanAudit(server.clan_id, {
                actor: body.userId,
                action: ClanAuditActions.DiscordRolesSetPosition,
                targetId: roleId,
                guildId,
                payload: {
                    guildId,
            ...
            logger.error(`[discord] set role position failed for ${guildId}/${roleId}: ${(err as Error).message}`);
            res.status(HTTP_INTERNAL_ERROR).json({ error: "set_role_position_failed" });
        }
    }),
);
