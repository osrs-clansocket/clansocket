...
import { discordWebhooksTopic } from "../../../data-rights/streams/topics/discord-webhooks-topic.js";
...
router.get("/:guildId/stream", requireSiteAccount, validateGuildId, (req: Request, res: Response) => {
    ...
    const handle = subscribeProjection(
        `discord_webhooks:${server.clan_id}:${guildId}`,
        discordWebhooksTopic(server.clan_id, guildId),
