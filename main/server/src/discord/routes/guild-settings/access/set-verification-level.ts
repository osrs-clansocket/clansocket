...
const CLANSOCKET_PERMISSION = "discord:guild-settings.set-verification-level";

interface SetVerificationLevelBody {
    userId: string;
    guildName: string;
    beforeLevel: number;
    level: number;
...
router.patch(
    ...
    handleAsync(async (req: Request, res: Response) => {
        ...
        const body = req.body as SetVerificationLevelBody;
        ...
        try {
            ...
            recordClanAudit(server.clan_id, {
                actor: body.userId,
                action: ClanAuditActions.DiscordGuildSettingsSetVerificationLevel,
                targetId: guildId,
                guildId,
                payload: {
                    guildId,
