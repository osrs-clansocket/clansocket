...
const CLANSOCKET_PERMISSION = "discord:guild-settings.set-name";
...
router.patch(
    ...
    handleAsync(async (req: Request, res: Response) => {
        ...
        try {
            ...
            const beforeJson = JSON.stringify({ subject: SUBJECT_NAME, name: body.beforeName });
            const afterJson = JSON.stringify({ subject: SUBJECT_NAME, name: body.name });
            ...
            recordClanAudit(server.clan_id, {
                actor: body.userId,
                action: ClanAuditActions.DiscordGuildSettingsSetName,
                targetId: guildId,
                guildId,
                payload: { guildId, targetName: body.name, beforeName: body.beforeName, afterName: body.name },
            });
            res.status(HTTP_OK).json({ sessionId, changeId, queueId });
        } catch (err) {
            logger.error(`[discord] guild-settings set-name failed for ${guildId}: ${(err as Error).message}`);
            res.status(HTTP_INTERNAL_ERROR).json({ error: "set_name_failed" });
        }
    }),
);
