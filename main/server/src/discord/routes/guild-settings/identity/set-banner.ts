...
const CLANSOCKET_PERMISSION = "discord:guild-settings.set-banner";
    ...
    handleAsync(async (req: Request, res: Response) => {
        ...
        try {
            ...
            recordClanAudit(server.clan_id, {
                actor: body.userId,
                action: ClanAuditActions.DiscordGuildSettingsSetBanner,
                targetId: guildId,
                guildId,
                payload: {
                    guildId,
            ...
            logger.error(`[discord] guild-settings set-banner failed for ${guildId}: ${(err as Error).message}`);
            res.status(HTTP_INTERNAL_ERROR).json({ error: "set_banner_failed" });
        }
    }),
);
