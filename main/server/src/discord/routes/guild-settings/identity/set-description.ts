...
const CLANSOCKET_PERMISSION = "discord:guild-settings.set-description";
...
router.patch(
    ...
    handleAsync(async (req: Request, res: Response) => {
        ...
        try {
            ...
            const beforeJson = JSON.stringify({ subject: SUBJECT_DESCRIPTION, description: body.beforeDescription });
            const afterJson = JSON.stringify({ subject: SUBJECT_DESCRIPTION, description: body.description });
            ...
            recordClanAudit(server.clan_id, {
                actor: body.userId,
                action: ClanAuditActions.DiscordGuildSettingsSetDescription,
                targetId: guildId,
                guildId,
                payload: {
                    guildId,
            ...
            logger.error(`[discord] guild-settings set-description failed for ${guildId}: ${(err as Error).message}`);
            res.status(HTTP_INTERNAL_ERROR).json({ error: "set_description_failed" });
        }
    }),
);
