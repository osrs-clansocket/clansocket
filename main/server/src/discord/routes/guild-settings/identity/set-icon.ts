...
const CLANSOCKET_PERMISSION = "discord:guild-settings.set-icon";
...
router.patch(
    ...
    handleAsync(async (req: Request, res: Response) => {
        ...
        try {
            ...
            const beforeJson = JSON.stringify({ subject: SUBJECT_ICON, iconUrl: body.beforeIconUrl });
            const afterJson = JSON.stringify({ subject: SUBJECT_ICON, iconDataUrl: body.iconDataUrl });
            ...
            recordClanAudit(server.clan_id, {
                actor: body.userId,
                action: ClanAuditActions.DiscordGuildSettingsSetIcon,
                targetId: guildId,
                guildId,
                payload: {
                    guildId,
            ...
            logger.error(`[discord] guild-settings set-icon failed for ${guildId}: ${(err as Error).message}`);
            res.status(HTTP_INTERNAL_ERROR).json({ error: "set_icon_failed" });
        }
    }),
);
