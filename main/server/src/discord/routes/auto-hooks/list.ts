...
router.get(
    ...
    handleAsync(async (req: Request, res: Response) => {
        ...
        try {
            const autoHooks = listAutoHooks(server.clan_id, guildId);
            res.status(HTTP_OK).json({ autoHooks });
        } catch (err) {
            logger.error(`[discord] auto-hooks list failed for ${guildId}: ${(err as Error).message}`);
            res.status(HTTP_INTERNAL_ERROR).json({ error: "auto_hooks_list_failed" });
        }
    }),
);
