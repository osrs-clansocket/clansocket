...
router.get(
    ...
    handleAsync(async (req: Request, res: Response) => {
        ...
        try {
            const members = listMembersForGuild(server.clan_id, guildId);
            res.status(HTTP_OK).json({ members });
        } catch (err) {
            logger.error(`[discord] members list failed for ${guildId}: ${(err as Error).message}`);
            res.status(HTTP_INTERNAL_ERROR).json({ error: "members_list_failed" });
        }
    }),
);
