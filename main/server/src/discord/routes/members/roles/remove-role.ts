...
const CLANSOCKET_PERMISSION = "discord:members.remove-role";
...
router.delete(
    ...
    handleAsync(async (req: Request, res: Response) => {
        ...
        const body = req.body as RemoveRoleBody;
        ...
        try {
            ...
            recordClanAudit(server.clan_id, {
                actor: body.userId,
                action: ClanAuditActions.DiscordMembersRemoveRole,
            ...
            logger.error(`[discord] members remove-role failed for ${guildId}: ${(err as Error).message}`);
            res.status(HTTP_INTERNAL_ERROR).json({ error: "remove_role_failed" });
        }
    }),
);
