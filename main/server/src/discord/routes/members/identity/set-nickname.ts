...
const RATE_LIMIT_ROUTE = "/guilds/:id/members/:user_id";
const CLANSOCKET_PERMISSION = "discord:members.set-nickname";
...
router.patch(
    "/:guildId/:userId/nickname",
    validateGuildId,
    handleAsync(async (req: Request, res: Response) => {
        const guildId = req.params.guildId as string;
        const targetUserIdParam = req.params.userId as string;
        ...
        try {
            ...
            logger.error(`[discord] members set-nickname failed for ${guildId}: ${(err as Error).message}`);
            res.status(HTTP_INTERNAL_ERROR).json({ error: "set_nickname_failed" });
        }
    }),
);
