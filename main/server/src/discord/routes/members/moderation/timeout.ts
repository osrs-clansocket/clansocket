...
const CLANSOCKET_PERMISSION = "discord:members.timeout";
    ...
    handleAsync(async (req: Request, res: Response) => {
        ...
        try {
            ...
            recordClanAudit(server.clan_id, {
                actor: body.userId,
                action: ClanAuditActions.DiscordMembersTimeout,
            ...
            logger.error(`[discord] members timeout failed for ${guildId}: ${(err as Error).message}`);
            res.status(HTTP_INTERNAL_ERROR).json({ error: "timeout_failed" });
        }
    }),
);
