...
const CLANSOCKET_PERMISSION = "discord:members.unban";
    ...
    handleAsync(async (req: Request, res: Response) => {
        ...
        try {
            ...
            recordClanAudit(server.clan_id, {
                actor: body.userId,
                action: ClanAuditActions.DiscordMembersUnban,
            ...
            logger.error(`[discord] members unban failed for ${guildId}: ${(err as Error).message}`);
            res.status(HTTP_INTERNAL_ERROR).json({ error: "unban_failed" });
        }
    }),
);
