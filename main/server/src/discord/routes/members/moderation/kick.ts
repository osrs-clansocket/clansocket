...
const CLANSOCKET_PERMISSION = "discord:members.kick";
    ...
    handleAsync(async (req: Request, res: Response) => {
        ...
        try {
            ...
            recordClanAudit(server.clan_id, {
                actor: body.userId,
                action: ClanAuditActions.DiscordMembersKick,
            ...
            logger.error(`[discord] members kick failed for ${guildId}: ${(err as Error).message}`);
            res.status(HTTP_INTERNAL_ERROR).json({ error: "kick_failed" });
        }
    }),
);
