...
const RATE_LIMIT_ROUTE = "/guilds/:id/emojis";
const CLANSOCKET_PERMISSION = "discord:server-emojis.create";

interface CreateServerEmojiBody {
    userId: string;
    name: string;
    imageDataUrl: string;
    animated: boolean;
...
router.post(
    ...
    handleAsync(async (req: Request, res: Response) => {
        ...
        const body = req.body as CreateServerEmojiBody;
        ...
        try {
            ...
            recordClanAudit(server.clan_id, {
                actor: body.userId,
                action: ClanAuditActions.DiscordServerEmojisCreate,
                targetId: tempId,
                guildId,
                payload: {
                    guildId,
            ...
            logger.error(`[discord] server emoji create failed for ${guildId}: ${(err as Error).message}`);
            res.status(HTTP_INTERNAL_ERROR).json({ error: "server_emoji_create_failed" });
        }
    }),
);
