...
const CLANSOCKET_PERMISSION = "discord:channels.delete";
...
router.delete(
    ...
    handleAsync(async (req: Request, res: Response) => {
        ...
        try {
            ...
            const beforeJson = JSON.stringify({ name: body.channelName, channelType: body.channelType });
            ...
            recordClanAudit(server.clan_id, {
                actor: body.userId,
                action: ClanAuditActions.DiscordChannelsDelete,
                targetId: channelId,
                guildId,
                payload: { guildId, targetName: body.channelName, channelType: body.channelType },
            });
            res.status(HTTP_OK).json({ sessionId, changeId, queueId });
        } catch (err) {
            logger.error(`[discord] delete channel failed for ${guildId}/${channelId}: ${(err as Error).message}`);
            res.status(HTTP_INTERNAL_ERROR).json({ error: "delete_channel_failed" });
        }
    }),
);
