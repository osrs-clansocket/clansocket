import { getDb, DB_NAMES, getClanDb, getClanPluginDb, listClanPluginModes } from "../../core/database.js";
...
function hasPluginAccountsRow(clanId: string, hashes: readonly string[]): boolean {
    if (hashes.length === 0) return false;
    const ph = hashes.map(() => "?").join(",");
    for (const mode of listClanPluginModes(clanId)) {
        const row = getClanPluginDb(clanId, mode)
            .prepare(`SELECT 1 FROM plugin_accounts WHERE account_hash IN (${ph}) LIMIT 1`)
            .get(...hashes);
        if (row) return true;
    }
    return false;
...
// has no owner/manager role AND no plugin_accounts presence — that user has
...
export function resolveClanPosture(siteAccountId: string, clanId: string): ClanPosture | null {
    return resolvePostureWith(siteAccountId, clanId, hasPluginAccountsRow);
}
...
// plugin_accounts. Ex-members lose visibility automatically when the roster drops
